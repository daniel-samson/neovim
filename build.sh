...
cd $SRC_PATH;
cat utility/*.sh neovim/*.sh debian/utility/*.sh debian/packages/*sh debian/environments/*.sh debian/*.sh deepin/*.sh macos/*.sh macos/environments/*.sh macos/packages/*.sh detect_os.sh install.sh >> $BUILD_PATH/$INSTALL_SCRIPT;
cd $SCRIPTPATH;
